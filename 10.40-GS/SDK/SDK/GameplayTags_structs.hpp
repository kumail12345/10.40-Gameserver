...
public:
	...
	inline bool HasAll(const struct FGameplayTagContainer& ContainerToCheck) const
	{
	...
	bool HasAll(const FString& ContainerToCheckAsString) const;

	inline bool HasAny(const struct FGameplayTagContainer& ContainerToCheck) const
	{
		if (ContainerToCheck.IsEmpty())
		{
			return false;
		}
		for (const struct FGameplayTag& OtherTag : ContainerToCheck.GameplayTags)
		{
			if (GameplayTags.Contains(OtherTag) || ParentTags.Contains(OtherTag))
			{
				return true;
	...
	inline bool HasTag(const struct FGameplayTag& TagToCheck) const
	{
